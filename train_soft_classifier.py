...
from preprocessing_utils import load, load_classnames, load_labels, load_probs, load_text
...
class DProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for i, (text, label) in enumerate(zip(load_text(os.path.join(data_dir, self.train_dir)),
                                              load_probs(os.path.join(data_dir, self.train_dir)))):
            guid = "%s-%s" % ("train", i)
...
def train(args, train_dataset, model, tokenizer):
    ...
    # NEW: Use KL-Divergence as loss function instead of negative log-likelihood
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            pred_probs = nn.functional.log_softmax(logits, dim=1)
            loss = kl_loss(pred_probs, target)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                logits = outputs[0]
                pred_probs = nn.functional.log_softmax(logits, dim=1)
                tmp_eval_loss = kl_loss(pred_probs, target)
            ...
            if preds is None:
                ...
                hard_labels = torch.argmax(batch[3], dim=1)
                out_label_ids = hard_labels.detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                probs = np.append(probs, nn.functional.softmax(logits, dim=1).detach().cpu().numpy(), axis=0)
                hard_labels = torch.argmax(batch[3], dim=1)
                out_label_ids = np.append(out_label_ids, hard_labels.detach().cpu().numpy(), axis=0)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples, labels = processor.get_test_examples(args.test_data_dir, args.dataset_name) if evaluate else processor.get_train_examples(args.train_data_dir)
        examples = (examples)
    ...
    if output_mode == "classification":
        # all_labels = torch.tensor([torch.FloatTensor(f.label) for f in features], dtype=torch.long)
